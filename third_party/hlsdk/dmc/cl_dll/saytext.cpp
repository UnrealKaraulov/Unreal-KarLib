...
int CHudSayText :: Draw( float flTime )
{
	...
	flScrollTime = oldmin( flScrollTime, flTime + m_HUD_saytext_time->value );
	...
	for ( int i = 0; i < MAX_LINES; i++ )
	{
		if ( *g_szLineBuffer[i] )
		{
			if ( *g_szLineBuffer[i] == 2 && g_pflNameColors[i] )
			{
				...
				strncpy( buf, g_szLineBuffer[i], oldmin(g_iNameLengths[i], MAX_PLAYER_NAME_LENGTH+32) );
				buf[ oldmin(g_iNameLengths[i], MAX_PLAYER_NAME_LENGTH+31) ] = 0;
...
void CHudSayText :: SayTextPrint( const char *pszBuf, int iBufSize, int clientIndex )
{
	...
	strncpy( g_szLineBuffer[i], pszBuf, oldmax(iBufSize -1, MAX_CHARS_PER_LINE-1) );
