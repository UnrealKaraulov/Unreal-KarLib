...
{
	return g_benchSwitchTimes[ oldmin( Bench_GetStage(), LAST_STAGE ) ];
}
...
int CHudBenchmark::MsgFunc_Bench(const char *pszName, int iSize, void *pbuf)
{
	...
	m_StoredLatency = oldmin( 1.0, m_StoredLatency );
	m_StoredLatency = oldmax( 0.0, m_StoredLatency );
...
void CHudBenchmark::Think( void )
{
	...
	if ( Bench_InStage( FIRST_STAGE ) )
	{
		...
		if ( m_fReceiveTime )
		{
			...
			latency = oldmax( 0.0, latency );
			latency = oldmin( 1.0, latency );
	...
	if ( Bench_InStage( SECOND_STAGE ) )
	{
		...
		if ( total )
		{
			...
			frac *= 2.0;
			frac = oldmax( 0.0, frac );
			frac = oldmin( 1.0, frac );
...
int CHudBenchmark::Bench_ScoreForValue( int stage, float raw )
{
	...
	case 1:  // ping
		...
		score = oldmax( score, 0 );
		score = oldmin( score, 100 );
...
void CHudBenchmark::SetCompositeScore( void )
{
	...
	composite = oldmin( 100, composite );
	composite = oldmax( 0, composite );
	...
	if ( Bench_InStage( THIRD_STAGE ) && strstr( modelname, "ppdemodot" ) )
	{
		...
		if ( Bench_Active() && Bench_InStage( THIRD_STAGE ) )
		{
			...
			if ( dt > 0.0 && dt < 1.0 )
			{
				...
				fZAdjust = oldmin( fBounds, fZAdjust );
				fZAdjust = oldmax( -fBounds, fZAdjust );
		...
		*/
		...
		if ( ( NUM_BENCH_OBJ - i - 1 ) < gHUD.m_Benchmark.GetObjects() )
		{
			...
			else
			{
				...
				bench[i].curstate.renderamt = oldmin( 255, bench[i].curstate.renderamt );
	...
	if ( Bench_InStage( SECOND_STAGE ) || Bench_InStage( THIRD_STAGE ) )
	{
		...
		if ( recalc_wander )
		{
			...
			for ( i = 0; i < 2; i++ )
			{
				...
				v_stochastic[ i ] = oldmax( -15.0, v_stochastic[ i ] );
				v_stochastic[ i ] = oldmin( 15.0, v_stochastic[ i ] );
			}
